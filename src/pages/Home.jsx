...
import { Plane, Zap, Building2, Activity, Calendar, TrendingUp, Eye, Clock, MapPin, Battery, Signal } from 'lucide-react';
...
const Dashboard = () => {
  ...
  const recentFlights = [
    { id: 'FL001', route: 'NYC → LAX', status: 'In Flight', time: '14:30', aircraft: 'Boeing 737' },
    { id: 'FL002', route: 'CHI → MIA', status: 'Landed', time: '13:45', aircraft: 'Airbus A320' },
    { id: 'FL003', route: 'SEA → DEN', status: 'Scheduled', time: '16:20', aircraft: 'Boeing 777' },
    { id: 'FL004', route: 'ATL → BOS', status: 'Delayed', time: '12:15', aircraft: 'Embraer 190' }
  ];
  ...
  const recentPlans = [
    { id: 'PLN001', title: 'Morning Patrol Route', type: 'Surveillance', scheduled: '06:00', duration: '3h', status: 'Active' },
    { id: 'PLN002', title: 'Cargo Delivery - Zone B', type: 'Logistics', scheduled: '09:30', duration: '1h 30m', status: 'Pending' },
    { id: 'PLN003', title: 'Emergency Response Drill', type: 'Training', scheduled: '14:00', duration: '2h', status: 'Completed' },
    { id: 'PLN004', title: 'Infrastructure Inspection', type: 'Survey', scheduled: '16:45', duration: '2h 15m', status: 'Scheduled' }
  ];
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setLiveCounts(prev => ({
        ...
        activeDrones: Math.max(0, prev.activeDrones + Math.floor(Math.random() * 3) - 1)
      }));
    }, 5000);
  ...
  const getStatusColor = (status) => {
    switch (status.toLowerCase()) {
      case 'active': case 'in flight': return 'text-green-600 bg-green-100';
      case 'landed': case 'completed': return 'text-blue-600 bg-blue-100';
      case 'delayed': case 'charging': return 'text-yellow-600 bg-yellow-100';
      case 'returning': case 'pending': return 'text-orange-600 bg-orange-100';
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50 p-6">
      <div className="max-w-7xl mx-auto space-y-6">
        <div className="text-center mb-8">
          <h1 className="text-4xl font-bold text-gray-900 mb-2">Aviation Control Dashboard</h1>
          <p className="text-gray-600">Real-time monitoring and analytics for flights, drones, and aircraft</p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <DataCard title="Recent Flights" onViewAll={() => alert('Opening full flights data...')}>
              ...
              {recentFlights.map(flight => (
                <div key={flight.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="font-medium text-gray-900">{flight.route}</p>
          ...
          <DataCard title="Weekly Flight Activity" onViewAll={() => alert('Opening detailed analytics...')}>
                ...
                <div key={day.day} className="flex items-center gap-4">
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <span className="text-sm text-gray-600 w-12">{day.flights}</span>
                    </div>
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm text-gray-600 w-12">{day.drones}</span>
                    </div>
                  </div>
                </div>
              ))}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <DataCard title="Recent Flight Plans" onViewAll={() => alert('Opening all flight plans...')}>
            <div className="space-y-3">
                ...
                <div key={plan.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-600">{plan.type} • {plan.duration}</p>
                    </div>
                  </div>
                  <div className="text-right">
                    ...
                    <p className="text-sm text-gray-600 mt-1">{plan.scheduled}</p>
                  </div>
                </div>
              ))}
            </div>
          ...
          <DataCard title="Drone Fleet Overview" onViewAll={() => alert('Opening detailed drone analytics...')}>
            <div className="space-y-4">
        ...
        <DataCard title="Detailed Drone Performance Data" onViewAll={() => alert('Opening comprehensive drone data...')}>
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead>
                <tr className="border-b border-gray-200">
                  <th className="text-left py-3 px-4 font-medium text-gray-600">Drone</th>
                  <th className="text-left py-3 px-4 font-medium text-gray-600">Status</th>
                  <th className="text-left py-3 px-4 font-medium text-gray-600">Location</th>
                  <th className="text-left py-3 px-4 font-medium text-gray-600">Performance Metrics</th>
                  ...
                  <tr key={drone.id} className="border-b border-gray-100 hover:bg-gray-50">
                    ...
                    <td className="py-4 px-4">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm text-gray-700">{drone.location}</span>
                      </div>
                    </td>
                    <td className="py-4 px-4">
